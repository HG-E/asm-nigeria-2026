...
## 17. Notification sending + Phase 4 (Review) — built

Both pieces from §17 (previous revision) landed together, since testing multi-reviewer routing meaningfully required the review workflow to exist.

**Product-owner feedback addressed**: can a subtheme have more than one reviewer? Yes — `reviewer_profiles`' only unique constraint is `(user_id, conference_id)`, nothing scopes `subtheme_id` to one reviewer. No schema change needed; `submit_abstract()` now loops over every active reviewer on the submission's subtheme and creates a `review_assignments` row (and a `reviewer_assignment` notification) for each — verified with two reviewers on the same subtheme, both correctly assigned.

**Notification sending** (`lib/email.ts`, `lib/notifications.ts`): Nodemailer over the same Gmail credentials as §16. Templates exist for `submission_acknowledgement` and `reviewer_assignment`; anything else falls back to a generic message. Two real issues found while wiring this in:

- Sending 2–3 emails synchronously before redirecting the author took 30+ seconds — bad UX and a serverless-timeout risk. Fixed with Next.js's `after()`: the redirect fires immediately, emails send in the background once the response is already on its way.
- Port 587 (STARTTLS) is blocked outbound from this dev sandbox (`connect ETIMEDOUT`); port 465 (implicit TLS) works and is also the more broadly serverless-safe choice. Switched `EMAIL_PORT` to 465 in both `.env.local` and Vercel — verified real delivery end-to-end afterward (`status: 'sent'` in the `notifications` table, not just no-error).

**Reviewer workflow** (`/reviewer/dashboard`, `/reviewer/assignments/[id]`) — spec §24–28: assigned/pending/completed/overdue summary, conflict-of-interest gate before any submission content is visible, then the double-blind abstract view (no author identity — RLS already enforced this as of migration 0002/0007, confirmed by direct check that a reviewer's page never contains the author's name) plus a 5-criterion 1–5 scoring form, recommendation, and locked-after-submit review.

**Two real bugs found and fixed while building/testing this, both would have blocked every review submission in production:**

1. Passed inline arrow functions (`(data) => saveReviewAction(id, data, false)`) as props from the Server Component page to the `ReviewForm` Client Component — Next.js can't serialize an arbitrary closure across that boundary (`Error: Event handlers cannot be passed to Client Component props`). Fixed by splitting into two separately-exported, independently-bindable actions (`saveReviewDraftAction`, `submitReviewAction`) instead of one action with a boolean flag.
2. The `reviews` table's RLS policy had `USING`/`WITH CHECK` backwards: `WITH CHECK (NOT is_submitted)` validates the row's state *after* the write, so it blocked the actual `is_submitted: false → true` submit transition itself — not just edits after submission, which was presumably the intent. No reviewer could have ever successfully submitted a review through this policy. Fixed in `0008_fix_reviews_lock_policy.sql` by moving the gate to `USING` (checked against the *old* row, so a submitted review stops matching for further writes) and leaving `WITH CHECK` to just verify ownership.

**Admin manual reassignment** (spec §23) added to the submission detail page — add/remove reviewers from a submission outside the automatic routing, for conflict-of-interest replacements or corrections.

**Verified end-to-end** with a self-contained script: two disposable reviewers on the same subtheme, a real author submission through the full wizard, confirmed both got auto-assigned with a due date, confirmed the author's acknowledgement email actually sent, one reviewer declared a conflict (verified locked, no further action possible), the other completed a full review (verified double-blind — no author name anywhere on the page — and verified locked after submit), and admin correctly saw both statuses and could remove an assignment. Discovered the two bugs above specifically because this test exercised the real submit path rather than just checking that pages render. All test data (accounts, submission, reviewer_profiles) deleted afterward.

**Known local-only issue, not a code defect**: the Turbopack dev server started crashing (`0xc0000142` spawning a CSS-processing subprocess) partway through this session's testing, after many forceful `taskkill` restarts — `next build` continued to succeed cleanly throughout, including after the crashes started, which is why the admin-reassignment-panel verification and final testing moved to checking behavior directly rather than fighting the local dev server further.

## 18. Immediate next steps

Phase 5 (Committee decision workflow) is the next unbuilt piece: committee dashboard, review monitoring, and the accept/revision/reject decision flow — `decisions` table and its RLS already exist (committee proposes, admin/super_admin countersigns `is_final`, per §9), but there's no UI for it yet. Decision notifications (accepted/revision/rejected email content) can be added to `lib/notifications.ts` alongside that. Real reviewer names from the project owner would let §17's routing get exercised for real rather than just via test accounts.
